class JS_Doctor {
	 ...
	 * Danh sách
	 */
	loadList(offset = 1, limit = 15) {
		let myClass = this;
		myClass.offset = offset;
		myClass.limit = limit;
		...
		$.ajax({
			...
			success: function (result) {
				...
				$(myClass.oFormIndex).find('.main_paginate .pagination a').click(function () {
					...
					let limit = $('#cbo_nuber_record_page').val();
					myClass.loadList(page, limit);
				});
				$(myClass.oFormIndex).find('#cbo_nuber_record_page').change(function () {
					let page = $(myClass.oFormIndex).find('#_offset').val();
					let limit = $(myClass.oFormIndex).find('#cbo_nuber_record_page').val();
					myClass.loadList(page, limit);
				});
				$(myClass.oFormIndex).find('#cbo_nuber_record_page').val(result['limit']);
			}, error: function (e) {
				console.log(e);
				Library.hideloadding();
			}
		...
		$.ajax({
			...
			success: function (result) {
				...
				if (result['status'] == true) {
					...
					if (type) {
						$(".modal").modal('hide');
						myClass.loadList(myClass.offset, myClass.limit);
					}
				} else {
					Library.alertMessage('danger', 'Lỗi', result['message']);
				}
	...
	changeStatus(id) {
		let myClass = this;
		let url = `${myClass.urlPath}/changeStatus/${id}`;
		let data = {
			_token: $("#_token").val(),
